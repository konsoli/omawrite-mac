...
void Backend::printDocument() {
    ...
    if (dialog.exec() == QDialog::Accepted) {
        // The editor's font uses a pixel size (Main.qml's font.pixelSize), which
        // has no defined physical size on a paint device this document is never
        // attached to — Qt can't scale it against the printer's DPI, so
        // pagination collapses onto a small corner of the page. Print at a fixed
        // point size instead, which is resolution-independent.
        QFont printFont = m_document->defaultFont();
        if (printFont.pixelSize() > 0)
            printFont.setPointSizeF(11.0);
...
void Backend::applyDefaultTheme() {
